...
void Aircraft::def_guidance()
{
	...
	aircraft[14].init("man_start",0,"Manuever start time - sec","guidance","data","");
	aircraft[15].init("man_stop",0,"Manuever stop time - sec","guidance","data","");
...
///////////////////////////////////////////////////////////////////////////////
void Aircraft::guidance(Packet *combus,int num_vehicles, int vehicle_slot,double int_step)
{
	//local variables
	...
	double man_start=aircraft[14].real();
	double man_stop=aircraft[15].real();
	...
	//maneuver start and stop
	if((man_start<time)&&(time<man_stop))
	{
		//horizontal g-turn
		if(acft_option==1){
			Matrix ACOMV(3,3);
		...
		if(acft_option==2){
			//downloading from 'combus' missile states
			int i(0);
			char number[4];	
			Matrix STEL(3,1);
			Matrix VTEL(3,1);
			//building missile id = t(j+1); first missile is used
			sprintf(number,"%i",1);
			string rocket_id="m"+string(number);
			//finding slot 'i' of missile in 'combus' (same as in vehicle_list)
			for(i=0;i<num_vehicles;i++){
				string id=combus[i].get_id();
				if (id==rocket_id){
					//downloading data from missile packet
					tgt_com_slot=i;
			...
			//distance to missile
			Matrix SATL(3,1);
			//displacement vector of target aircraft wrt missile
			SATL=SAEL-STEL;
			...
			//escape maneuver; aircraft aligns with missile velocity vector
...
void Aircraft::def_control()
{
	...
	aircraft[24].init("phiavx",0,"Bank angle - deg","control","out","com");
	aircraft[25].init("phiavcx",0,"Commanded bank angle - deg","control","diag","");
	aircraft[26].init("anx",0,"Normal load factor - g's","control","state","com");
