///////////////////////////////////////////////////////////////////////////////
//FILE: 'rocket_functions.cpp'
//
// Contains utilitiy functions for the 'Rocket' class:
//		array sizing
...
///////////////////////////////////////////////////////////////////////////////
//Determining dimensions of arrays: 'com_rocket'
// 
//Out to Rocket:: flat3_com_count, rocket_com_count, ncom_rocket5 		 ,
//
//010207 Created by Peter H Zipfel
//170914 Converted form 'target' to 'rocket'
///////////////////////////////////////////////////////////////////////////////
void Rocket::sizing_arrays()
{
	char *key4="com";
	int i(0);
	...
	//counting in 'rocket' array
 	for(i=0;i<NROCKET;i++)
	{
		if(strstr(rocket[i].get_out(),key4))
			rocket_com_count++;
	}

	//output to Rocket::protected
	ncom_rocket5=flat3_com_count+rocket_com_count;
}
///////////////////////////////////////////////////////////////////////////////
//Reading input data from input file 'input.asc' for each vehicle object
//Assigning initial values to module-variables of 'flat3' and 'rocket' arrays
//Reading aero and propulsion decks
//
//The first vehicle 'ROCKET5' of 'input.asc' reads until the first 'END' after
//'ROCKET5'. The second vehicle object reads untile the second 'END', etc until
//the data for all vehicle objects are read
//
//Output:	rocket3_name ('Rocket' data member)
//			flat3[] data values (Flat3 data member)
//			rocket[] data values ('Rocket' data member)
...
///////////////////////////////////////////////////////////////////////////////
void Rocket::vehicle_data(fstream &input,int nmonte)
{
	...
	input.getline(rocket3_name,CHARL,'\n');
	...
	do
	{
		...
		else
		{
			...
			for(i=0;i<NROCKET;i++)
			{
				buff=rocket[i].get_name();
				if(!strcmp(buff,read)) 
				{
					input>>data;
					//checking for integers
					integer=rocket[i].get_type();
					if(!strcmp(integer,"int"))
					{
						//loading interger value
						int_data=(int)data;
						rocket[i].gets(int_data);
						input.getline(line_clear,CHARL,'\n');
					}
					else
					{
			...
			if(!strcmp(read,"UNI"))
			{
				...
				for(kk=0;kk<NROCKET;kk++)
				{
					buff=rocket[kk].get_name();
					if(!strcmp(buff,name1)) 
					{
						rocket[kk].gets(value);
						input.getline(line_clear,CHARL,'\n');
					}				
				}
			}
			...
			if(!strcmp(read,"MARKOV"))
			{
				...
				for(ii=0;ii<NROCKET;ii++)
				{
					buff=rocket[ii].get_name();
					if(!strcmp(buff,name1)) 
					{
						markov_list[nmarkov].set_markov_vehicle_index(ii);
						rocket[ii].gets(value);
						input.getline(line_clear,CHARL,'\n');
					}				
				}
				nmarkov++;						
...
///////////////////////////////////////////////////////////////////////////////
//Building index array of those 'flat3[]' and rocket[] variables 
//that are output to 'combus' 'data'  
//
//Output: Rocket::flat3_com_ind[], rocket_com_ind[] 
//
//010210 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
void Rocket::com_index_arrays()
{
	const char *test="com";
	int i(0);
	...
	for(i=0;i<NROCKET;i++)
	{
		if(strstr(rocket[i].get_out(),test))
		{
			rocket_com_ind[l]=i;
			l++;
		}
...
///////////////////////////////////////////////////////////////////////////////
//Initializing loading 'packet' of rocket
//
...
///////////////////////////////////////////////////////////////////////////////
Packet Rocket::loading_packet_init(int num_missile,int num_aircraft,int num_rocket,int num_radar)
{
	string id;
	char object[4];
	...
	c_count++;
	if(c_count==(num_rocket+1))c_count=1;
	...
	for(i=0;i<flat3_com_count;i++)
	{
		...
		com_rocket3[i]=flat3[index];
	}
	for(int j=0;j<rocket_com_count;j++)
	{
		index=rocket_com_ind[j];
		com_rocket3[i+j]=rocket[index];
	}
	//refreshing the packet
	packet.set_id(id);
	packet.set_status(1);
	packet.set_data(com_rocket3);
	packet.set_ndata(ncom_rocket5);

	return packet;
}
///////////////////////////////////////////////////////////////////////////////
//Loading 'packet' of rocket
//
//uses C-code 'sprintf' function to convert 'int' to 'char'
//
//010207 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
Packet Rocket::loading_packet(int num_missile,int num_aircraft,int num_rocket,int num_radar)
{
	int index(0);
	int i(0);
...
//Listing 'rocket' module-variable arrays
//
//010214 Created by Peter H Zipfel
//020911 Added module-variable error flagging, PZi
///////////////////////////////////////////////////////////////////////////////

void Rocket::document(ostream &fdoc,char *title,Document *doc_rocket5)
{
	int i(0);

	fdoc<<"\n\n                                       Rocket Module-Variable Array \n\n";
	fdoc<<"---------------------------------------------------------------------------------------------------------------------\n";
	...
	{
		for(int j=0;j<i;j++){
			if(!strcmp(rocket[i].get_name(),rocket[j].get_name())&&strcmp(rocket[i].get_name(),"empty"))
				rocket[i].put_error(name_error_code);
		}				
		if(!strcmp(rocket[i].get_error(),"A")) cout<<" *** Error code 'A': duplicate name in rocket[] array, see 'doc.asc' ***\n"; 
		if(!strcmp(rocket[i].get_error(),"*")) cout<<" *** Error code '*': duplicate location in rocket[] array, see 'doc.asc' ***\n"; 

		fdoc<<rocket[i].get_error();
		fdoc.setf(ios::left);
		fdoc.width(4);fdoc<<i;
		if(!strcmp(rocket[i].get_type(),"int"))
		{
			fdoc.width(15);fdoc<<rocket[i].get_name();
			fdoc.width(5);fdoc<<" int ";
		}
		else
		{
			fdoc.width(20);fdoc<<rocket[i].get_name();
		}
		fdoc.width(54);fdoc<<rocket[i].get_def();
		fdoc.width(13);fdoc<<rocket[i].get_mod();
		fdoc.width(10);fdoc<<rocket[i].get_role();
		fdoc<<rocket[i].get_out();
		fdoc<<"\n";
		if(!((i+1)%10))fdoc<<"----------------------------------------------------------------------------------------------------------------------\n";			
	}
	...
	//building doc_rocket5[] for documenting 'input.asc' and eliminating 'empty' slots
	int counter=0;
	for(i=0;i<NROCKET;i++){
		if(strcmp(rocket[i].get_name(),"empty")){
			doc_rocket5[counter].put_doc_offset(counter);
			doc_rocket5[counter].put_name(rocket[i].get_name());
			doc_rocket5[counter].put_type(rocket[i].get_type());
			doc_rocket5[counter].put_def(rocket[i].get_def());
			doc_rocket5[counter].put_mod(rocket[i].get_mod());
			counter++;
		}
	}
	for(i=0;i<NFLAT3;i++){
		if(strcmp(flat3[i].get_name(),"empty")){
			doc_rocket5[counter].put_doc_offset(counter);
			doc_rocket5[counter].put_name(flat3[i].get_name());
			doc_rocket5[counter].put_type(flat3[i].get_type());
			doc_rocket5[counter].put_def(flat3[i].get_def());
			doc_rocket5[counter].put_mod(flat3[i].get_mod());
			counter++;
		}
	}
}
...
//070419 Included here in rocket_functions.cpp', PZi
///////////////////////////////////////////////////////////////////////////////
void Rocket::read_tables(char *file_name,Datadeck &datatable)
{
	char line_clear[CHARL];
